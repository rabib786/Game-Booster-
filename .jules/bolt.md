...
## 2024-05-24 - Accurate CPU Polling Interval
**Learning:** When polling CPU usage continuously via `psutil.cpu_percent(interval=None)`, it provides inaccurate instantaneous measurements reflecting elapsed time since import. Using a small blocking interval provides accurate readings.
**Action:** Use a small blocking interval (e.g., `interval=0.1`) in high-frequency backend telemetry polling.
